...
/// Adding ``Cruise._legs``
enum TripSchemaV16: VersionedSchema {
    static var versionIdentifier = Schema.Version(2, 3, 3)
...
extension ModelContext {
    func migrateToV16() throws {
        logger.info("Begin migrating to V16")
        let trips = try self.fetch(TripSchemaV16.Trip.self)
        for trip in trips {
            trip.id = .init()
        }
        let soundings = try self.fetch(TripSchemaV16.Sounding.self)
