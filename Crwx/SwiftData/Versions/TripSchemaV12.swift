...
enum TripSchemaV12: VersionedSchema {
    static var versionIdentifier = Schema.Version(2, 2, 3)
...
extension TripMigrationPlan {
    static let migrateV11toV12: MigrationStage = .custom(fromVersion: TripSchemaV11.self, toVersion: TripSchemaV12.self, willMigrate: nil) { context in
        context.migrateToV12()
    }
}
...
@ModelActor
fileprivate final actor V12Migration {
    func migrate() async throws {
        let actor = RouteLoader(modelContainer: modelContainer)
        
        // 1. Get all trips.
        let trips = try modelContext.fetch(TripSchemaV12.Trip.self)
        for trip in trips {
            
            // 2. Find harbours for the two ends.
            let startHarbour = TripSchemaV12.Harbour.at(location: trip.departureLocation2, in: modelContext)
            let endHarbour = TripSchemaV12.Harbour.at(location: trip.arrivalLocation2, in: modelContext)
            startHarbour?.add(child: trip, to: \.trips)
            trip.add(child: startHarbour, to: \.harbours)
            if endHarbour != startHarbour {
                endHarbour?.add(child: trip, to: \.trips)
                trip.add(child: endHarbour, to: \.harbours)
                
                // 3. Find route between them.
                if let start = startHarbour?.waypoint,
                   let end = endHarbour?.waypoint
                {
                    trip.route2 = try await actor.route(from: start.persistentModelID, to: end.persistentModelID)
                }
            }
...
fileprivate extension TripSchemaV12.Harbour {
    static func at(location: (any Mappable)?, in context: ModelContext) -> TripSchemaV12.Harbour? {
        guard let location else { return nil }
        return (try? context.fetch(.harboursNear2(point: location, radius: .init(value: 0.25, unit: .nauticalMiles))).sorted(by: {
            $0.coordinate2.distance(to: location) < $1.coordinate2.distance(to: location)
        }))?.first
    }
