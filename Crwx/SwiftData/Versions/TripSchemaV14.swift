...
enum TripSchemaV14: VersionedSchema {
    static var versionIdentifier = Schema.Version(2, 3, 1)
...
extension TripMigrationPlan {
    static let migrateV13toV14: MigrationStage = .custom(fromVersion: TripSchemaV13.self, toVersion: TripSchemaV14.self, willMigrate: nil) { context in
        context.migrateToV14()
    }
}
...
extension ModelContext {
    func migrateToV14() {
        logger.info("Begin migration to V14")
        do {
            let trips = try self.fetch(TripSchemaV14.Trip.self)
            for trip in trips {

                // I want to calculate overnight depths into trips
                if let arrivalDepth = trip.arrivalDepth,
                   let arrivalTide = TideSnapshotSnippet(decoding: trip._arrivalTideData)?.height,
                   let tides = [TidePredictionSnippet](decoding: trip._tidePredictionsData),
                   let lowTide = tides.last(where: {
                       !$0.isHi
                   })?.height,
                   let highTide = tides.last(where: {
                       $0.isHi
                   })?.height
                {
                    let chartDepth = arrivalDepth - arrivalTide
                    let minDepth = chartDepth + lowTide
                    let maxDepth = chartDepth + highTide
                    guard minDepth < maxDepth
                    else { throw "Min \(minDepth) should be less than max \(maxDepth)" }
                    trip._overnightDepths = (minDepth..<maxDepth).encoded
                }
                
                // I want to build fuel soundings from trips
                if let fuelStart = FuelSounding(decoding: trip._fuelStartData)?.gallons,
                   let startTime = trip.departureTime
                {
                    let new = TripSchemaV14.Sounding(date: startTime, value: fuelStart, note: "", _type: 0)
                    self.insert(new)
                }
                if let fuelEnd = FuelSounding(decoding: trip._fuelEndData)?.gallons,
                   let endTime = trip.arrivalTime
                {
                    let new = TripSchemaV14.Sounding(date: endTime, value: fuelEnd, note: "", _type: 0)
                    self.insert(new)
                }
            }
            try self.save()
            logger.info("Successfully migrate to V14")
        } catch {
            logger.critical("There was an error during migration to V13: \(error)")
        }
    }
}
